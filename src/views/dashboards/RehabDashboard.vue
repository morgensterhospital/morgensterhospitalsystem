    ...
    <div class="dashboard-content">
      ...
      <div class="welcome-header">
        <div class="welcome-info">
          ...
          <p class="welcome-subtitle">Rehabilitation Dashboard</p>
        </div>
      ...
      <!-- Rehabilitation Request Status Kanban Board -->
      <div class="kanban-section">
        <h2 class="kanban-title">Rehabilitation Request Status</h2>
        <div class="kanban-board">
          ...
          <div class="kanban-column incoming">
            ...
            <div class="column-content">
              ...
              >
                ...
                <div class="request-type">{{ request.procedureType }}</div>
                <div class="request-time">{{ formatTime(request.timestamp) }}</div>
                <div class="request-actions">
                  <button class="action-btn start" @click.stop="startRequest(request)">
          ...
          <div class="kanban-column pending">
            ...
            <div class="column-content">
              ...
              >
                ...
                <div class="request-time">{{ formatTime(request.timestamp) }}</div>
                <div class="request-actions">
                  <button class="action-btn complete" @click.stop="completeRequest(request)">
          ...
          <div class="kanban-column failed">
            <div class="column-header">
              ...
              <span class="count-badge">{{ failedRequests.length }}</span>
            </div>
            <div class="column-content">
              <div
                v-for="request in failedRequests"
                :key="request.id"
                class="request-card failed"
                @click="viewRequest(request)"
              >
                <div class="request-patient">{{ request.patientName }}</div>
                <div class="request-type">{{ request.procedureType }}</div>
                ...
                <div class="request-time">{{ formatTime(request.completedTimestamp) }}</div>
                <div class="request-actions">
                  <button class="action-btn retry" @click.stop="retryRequest(request)">
      ...
      <div class="reports-section">
        <div class="reports-card">
          <div class="reports-header">
            <h3>Rehabilitation Reports</h3>
            <button class="generate-btn" @click="downloadRehabReportPDF">
...
const incomingRequests = ref([
  ...
  },
  {
    ...
    procedureType: 'Occupational Therapy',
...
const doneRequests = ref([
  {
    ...
    procedureType: 'Physical Therapy',
    note: 'Good progress, continue exercises',
    completedTimestamp: new Date(Date.now() - 90 * 60 * 1000)
  }
])
...
const failedRequests = ref([
  {
    ...
    procedureType: 'Occupational Therapy',
    note: 'Patient did not attend session',
    completedTimestamp: new Date(Date.now() - 120 * 60 * 1000)
  }
])
...
const userDisplayName = computed(() => {
  ...
  return authStore.user?.displayName || authStore.user?.email?.split('@')[0] || 'Rehab Technician'
})
...
const startRequest = (request) => {
  ...
  if (index !== -1) {
    const requestItem = incomingRequests.value.splice(index, 1)[0]
    pendingRequests.value.push(requestItem)
  }
}
...
const completeRequest = (request) => {
  ...
  if (index !== -1) {
    const requestItem = pendingRequests.value.splice(index, 1)[0]
    requestItem.note = 'Rehabilitation session completed'
    requestItem.completedTimestamp = new Date()
    doneRequests.value.push(requestItem)
  }
}

const retryRequest = (request) => {
  console.log('Retrying request:', request.id)
  // Move from failed to incoming
  const index = failedRequests.value.findIndex(r => r.id === request.id)
  if (index !== -1) {
    const requestItem = failedRequests.value.splice(index, 1)[0]
    delete requestItem.note
    delete requestItem.completedTimestamp
    incomingRequests.value.push(requestItem)
  }
}
...
// Download rehabilitation report PDF
const downloadRehabReportPDF = async () => {
  try {
    await apiService.generateReportPDF('rehabilitation_report', reportDateFrom.value, reportDateTo.value, authStore.user.uid)
  } catch (error) {
    console.error('Error downloading rehabilitation report PDF:', error)
